...
| 单表多字段 | 20 | 100.0% | 95.0% | 9.3 | 100.0% | 100.0% | 29.7 |
| 单表 | 40 | 100.0% | 95.0% | 8.1 | 100.0% | 100.0% | 28.8 |
| 多表 | 37 | 94.6% | 51.3% | 13.4 | 100.0% | 67.6% | 30.9 |
| 合计 | 77 | 97.4% | 74.0% | 10.6 | 100.0% | 84.4% | 29.8 |
...
- first_hit_rank（列槽）：1
- top_tables（列槽@10）：['dwd_ev_indv_crd_app']
- hits：column:dwd_ev_indv_crd_app.app_cred_amt(0.6675), column:dwd_ev_indv_crd_app.apprv_cred_amt(0.6657), column:dwd_ev_indv_crd_app.apply_date(0.6212), column:dwd_ev_indv_crd_app.last_update_time(0.6193), column:dwd_ev_indv_crd_app.last_update_date(0.6179), column:dwd_ev_indv_crd_app.loan_purpose(0.6171), column:dwd_ev_indv_crd_app.app_no(0.6171), column:dwd_ev_indv_crd_app.cred_date_start(0.6111)
- 建议标签：`DESC_WEAK` / `SYNONYM` / `CONFUSION` / `MULTI_HOP` / `INDEX_GAP`
...
- hits：column:dwd_ev_indv_loan_app.marr_status(0.6205), column:dwd_ev_indv_loan_app.sex(0.7047), column:dwd_ev_indv_loan_app.app_loan_amt(0.6199), column:dwd_ev_indv_loan_app.cost_date_start(0.5948), column:dwd_ev_indv_loan_app.apprv_loan_amt(0.5947), column:dwd_ev_indv_loan_app.loan_app_no(0.5943), column:dwd_ev_indv_loan_app.loan_purpose(0.5921), column:dwd_ev_indv_loan_app.phone_no(0.5912)
- 建议标签：`DESC_WEAK` / `SYNONYM` / `CONFUSION` / `MULTI_HOP` / `INDEX_GAP`
...
- hits：column:dwd_ev_comp_crd_app.customername(0.6632), column:dwd_ev_comp_crd_app.productid(0.6597), column:dwd_ev_comp_crd_app.baseproduct(0.6429), column:dwd_ev_comp_crd_app.cusindustrytype(0.6324), column:dwd_ev_comp_crd_app.loantype(0.6312), column:dwd_ev_comp_crd_app.credittype(0.6312), column:dwd_ev_comp_crd_app.businesssum(0.631), column:dwd_ev_comp_crd_app.customerid(0.6267)
- 建议标签：`DESC_WEAK` / `SYNONYM` / `CONFUSION` / `MULTI_HOP` / `INDEX_GAP`
...
- first_hit_rank（列槽）：1
- top_tables（列槽@10）：['dwd_ev_indv_crd_app']
- hits：column:dwd_ev_indv_crd_app.apprv_cred_amt(0.6335), column:dwd_ev_indv_crd_app.diploma(0.7638), column:dwd_ev_indv_crd_app.app_cred_amt(0.626), column:dwd_ev_indv_crd_app.career(0.6178), column:dwd_ev_indv_crd_app.age(0.615), column:dwd_ev_indv_crd_app.cust_id(0.6142), column:dwd_ev_indv_crd_app.year_income(0.609), column:dwd_ev_indv_crd_app.cust_rate(0.608)
- 建议标签：`DESC_WEAK` / `SYNONYM` / `CONFUSION` / `MULTI_HOP` / `INDEX_GAP`
...
- first_hit_rank（列槽）：1
- top_tables（列槽@10）：['dwd_ev_indv_loan_app']
- hits：column:dwd_ev_indv_loan_app.marr_status(0.7776), column:dwd_ev_indv_loan_app.loan_app_no(0.6368), column:dwd_ev_indv_loan_app.loan_purpose(0.6357), column:dwd_ev_indv_loan_app.cost_date_start(0.6353), column:dwd_ev_indv_loan_app.phone_no(0.6208), column:dwd_ev_indv_loan_app.app_loan_amt(0.6182), column:dwd_ev_indv_loan_app.prd_code(0.6179), column:dwd_ev_indv_loan_app.cost_date_end(0.6167)
- 建议标签：`DESC_WEAK` / `SYNONYM` / `CONFUSION` / `MULTI_HOP` / `INDEX_GAP`
...
|------|------|-------|-----------|------------|------------|---------------|
| ALL | 77 | 92.2% | 66.2% | 95.7% | 100.0% | **35.7%** |
| single | 40 | 95.0% | 92.5% | 92.5% | 100.0% | **50.0%** |
| multi | 37 | 89.2% | 37.8% | 100.0% | 100.0% | **16.7%** |

### 过程指标（不计入主准确率）

- 首轮 plan 通过率：26.0%；平均 plan 尝试 1.81 次；平均 SQL 尝试 0.99 次
- 最终无 SQL：plan 耗尽 5 题，SQL 耗尽/生成失败 1 题
...
- `N_S03` 本科以上学历个人客户人数  pred_rows=0 gold_rows=1  err=result_mismatch
  pred:
  ```sql
  SELECT `dwd_ip_indv_cust_info`.`highest_schooling`, COUNT(`dwd_ip_indv_cust_info`.`cust_id`) FROM `dwd_ip_indv_cust_info` WHERE `dwd_ip_indv_cust_info`.`highest_schooling` IN ('本科', '硕士', '博士') GROUP BY `dwd_ip_indv_cust_info`.`highest_schooling` LIMIT 100
  ```
  gold:
  ```sql
  SELECT COUNT(DWD_IP_INDV_CUST_INFO.highest_schooling) AS `highest_schooling_count` FROM DWD_IP_INDV_CUST_INFO LIMIT 100
  ```
- `N_S04` 全部用信申请金额求和  pred_rows=1 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT SUM(dwd_ev_indv_loan_app.app_loan_amt) FROM dwd_ev_indv_loan_app LIMIT 100
  ```
  gold:
  ```sql
...
- `N_S07` 存在逾期的借据有多少  pred_rows=1 gold_rows=1  err=result_mismatch
  ...
  ```
- `N_S09` 各信贷产品宽限期天数  pred_rows=10 gold_rows=10  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_prd_info.grace_day AS `grace_day`, dwd_prd_info.prd_name AS `prd_name`, dwd_prd_info.prd_code AS `prd_code` FROM dwd_prd_info LIMIT 100
  ```
  gold:
  ```sql
  SELECT DWD_PRD_INFO.grace_day AS `grace_day` FROM DWD_PRD_INFO LIMIT 100
  ```
- `N_S11` 个人客户按性别分组统计  pred_rows=2 gold_rows=1  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_ip_indv_cust_info.sex AS `sex`, COUNT(dwd_ip_indv_cust_info.sex) AS `count` FROM dwd_ip_indv_cust_info GROUP BY dwd_ip_indv_cust_info.sex LIMIT 100
  ```
  gold:
  ```sql
...
- `N_S12` 借据层面宽限天数分布  pred_rows=None gold_rows=100  err=EXPLAIN 失败: (1140, "In aggregated query without GROUP BY, expression #2 of SELECT list contains nonaggregated column 'vectortest.dwd_ar_loan_info.grace_day'; this is incompatible with sql_mode=only_full_group_by")
  ...
  ```
- `N_S14` 逾期追偿场景下用户还款日期分布  pred_rows=1 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT COUNT(`practical_pay_date`) AS `repay_date_count` FROM `dwd_ev_overdue_repay` ORDER BY `practical_pay_date` ASC LIMIT 100
  ```
  gold:
  ```sql
...
- `N_S20` 信贷审批流程当前节点分布  pred_rows=None gold_rows=None  err=empty pred sql
  ...
  ```
- `N_S21` 个人客户婚姻与性别交叉统计  pred_rows=77 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_ip_indv_cust_info.marriage AS `marriage`, dwd_ip_indv_cust_info.sex AS `sex`, COUNT(cust_id) AS `count` FROM dwd_ip_indv_cust_info GROUP BY dwd_ip_indv_cust_info.marriage, dwd_ip_indv_cust_info.sex LIMIT 100
  ```
  gold:
  ```sql
...
- `N_S28` 职工人数超过100的企业名称  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT COUNT(dwd_ip_corp_cust_info.cust_name) AS `cust_name_count`, dwd_ip_corp_cust_info.employee_number AS `employee_number` FROM dwd_ip_corp_cust_info WHERE dwd_ip_corp_cust_info.employee_number > 100 GROUP BY dwd_ip_corp_cust_info.employee_number LIMIT 100
  ```
  gold:
  ```sql
...
- `N_S30` 已结清借据的结清日与借款人姓名  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT `dwd_ar_loan_info`.`name`, `dwd_ar_loan_info`.`payoff_date` FROM `dwd_ar_loan_info` WHERE `dwd_ar_loan_info`.`payoff_date` IS NOT NULL LIMIT 100
  ```
  gold:
  ```sql
...
- `N_S33` 授信申请日期与审批结果状态  pred_rows=100 gold_rows=100  err=result_mismatch
  ...
  ```
- `N_S39` 产品增信方式与授信类型  pred_rows=100 gold_rows=10  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_prd_info.credit_method AS `credit_method`, dwd_prd_info.credit_type AS `credit_type`, dwd_ev_indv_crd_app.prd_code AS `prd_code`, dwd_prd_info.prd_code AS `prd_code`, dwd_ev_indv_loan_pub.product_id AS `product_id`, dwd_ar_loan_info.prd_code AS `prd_code` FROM dwd_prd_info INNER JOIN dwd_ar_loan_info ON dwd_ar_loan_info.prd_code = dwd_pr
  ```
  gold:
  ```sql
  SELECT DWD_PRD_INFO.credit_method AS `credit_method`, DWD_PRD_INFO.credit_type AS `credit_type` FROM DWD_PRD_INFO LIMIT 100
  ```
- `N_M01` 35岁以上个人客户及其用信申请金额  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_ev_indv_crd_app.cust_id, dwd_ev_indv_crd_app.app_cred_amt FROM dwd_ev_indv_crd_app INNER JOIN dwd_ip_indv_cust_info ON dwd_ev_indv_crd_app.cust_id = dwd_ip_indv_cust_info.cust_id WHERE dwd_ip_indv_cust_info.age > 35 LIMIT 100
  ```
  gold:
  ```sql
  SELECT DWD_IP_INDV_CUST_INFO.age AS `age`, DWD_IP_INDV_CUST_INFO.cust_id AS `cust_id`, DWD_EV_INDV_LOAN_APP.app_loan_amt AS `app_loan_amt` FROM DWD_IP_INDV_CUST_INFO INNER JOIN dwd_ev_indv_loan_app ON dwd_ev_indv_loan_app.cust_id = dwd_ip_indv_cust_info.cust_id LIMIT 100
  ```
- `N_M02` 个人客户主档姓名手机与授信审批额度  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_ev_indv_crd_app.name, dwd_ev_indv_crd_app.phone_no, dwd_ev_indv_crd_app.apprv_cred_amt FROM dwd_ev_indv_crd_app LIMIT 100
  ```
  gold:
  ```sql
  SELECT DWD_IP_INDV_CUST_INFO.name AS `name`, DWD_IP_INDV_CUST_INFO.phone_no AS `phone_no`, DWD_EV_INDV_CRD_APP.apprv_cred_amt AS `apprv_cred_amt` FROM DWD_IP_INDV_CUST_INFO INNER JOIN dwd_ev_indv_crd_app ON dwd_ev_indv_crd_app.cust_id = dwd_ip_indv_cust_info.cust_id LIMIT 100
  ```
- `N_M03` 借据金额及关联还款计划应还本金  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_ev_repay_plan.rpy_princ, dwd_ar_loan_info.loan_no FROM dwd_ev_repay_plan INNER JOIN dwd_ar_loan_info ON dwd_ev_repay_plan.loan_no = dwd_ar_loan_info.loan_no LIMIT 100
  ```
  gold:
  ```sql
...
- `N_M04` 借据对应产品名称与本金余额  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_app_mapping.prd_name, dwd_ar_loan_info.prin_bal FROM dwd_ar_loan_info INNER JOIN dwd_app_mapping ON dwd_ar_loan_info.loan_no = dwd_app_mapping.loan_no LIMIT 100
  ```
  gold:
  ```sql
