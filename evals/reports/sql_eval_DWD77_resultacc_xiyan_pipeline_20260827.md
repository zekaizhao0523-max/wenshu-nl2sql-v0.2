...
| 多表 | 37 | 94.6% | 67.6% | 14.2 | 100.0% | 86.5% | 32.5 |
| 合计 | 77 | 96.1% | 80.5% | 11.2 | 100.0% | 92.2% | 31.1 |

- S1 来源：llm=1, llm_cover=76
- S2 来源：llm=1, llm_cover=52, llm_empty_widen_cover=24
...
|------|------|-------|-----------|------------|------------|---------------|
| ALL | 77 | 79.2% | 59.7% | 89.4% | 100.0% | **47.0%** |
| single | 40 | 82.5% | 75.0% | 86.1% | 100.0% | **61.1%** |
| multi | 37 | 75.7% | 43.2% | 93.3% | 100.0% | **30.0%** |

### 过程指标（不计入主准确率）

- 首轮 plan 通过率：28.8%；平均 plan 尝试 1.53 次；平均 SQL 尝试 0.83 次
- 最终无 SQL：plan 耗尽 4 题，SQL 耗尽/生成失败 1 题
...
- `N_S08` 逾期处置追偿本金总额  pred_rows=1 gold_rows=1  err=result_mismatch
  pred:
  ```sql
  SELECT SUM(prin_amt) FROM dwd_ev_overdue_repay WHERE alias_rpy_type = '追偿' LIMIT 100
  ```
  gold:
  ```sql
...
- `N_S24` 产品目录中的名称与编码清单  pred_rows=100 gold_rows=10  err=result_mismatch
  pred:
  ```sql
  SELECT `dwd_prd_info`.`prd_name` AS `prd_name`, `dwd_prd_info`.`prd_code` AS `prd_code`, `dwd_ev_indv_crd_app`.`prd_code` AS `prd_code`, `dwd_ev_indv_loan_pub`.`product_id` AS `product_id`, `dwd_ar_loan_info`.`prd_code` AS `prd_code`, `dwd_ev_indv_loan_app`.`prd_code` AS `prd_code` FROM `dwd_ar_loan_info` INNER JOIN `dwd_ev_indv_loan_pub` ON `dwd_a
  ```
  gold:
  ```sql
...
- `N_M01` 35岁以上个人客户及其用信申请金额  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_ev_indv_loan_app.app_loan_amt, dwd_ip_indv_cust_info.cust_id FROM dwd_ev_indv_loan_app INNER JOIN dwd_ip_indv_cust_info ON dwd_ev_indv_loan_app.cust_id = dwd_ip_indv_cust_info.cust_id WHERE dwd_ip_indv_cust_info.age > 35 LIMIT 100
  ```
  gold:
  ```sql
...
- `N_M07` 实还本金与实还日期按借据汇总  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT SUM(dwd_ev_repay_detail.princ) AS `实还本金`, MAX(dwd_ev_repay_detail.repay_date) AS `实还日期`, dwd_ar_loan_info.loan_no AS `借据` FROM dwd_ev_repay_detail INNER JOIN dwd_ar_loan_info ON dwd_ev_repay_detail.loan_no = dwd_ar_loan_info.loan_no GROUP BY dwd_ar_loan_info.loan_no LIMIT 100
  ```
  gold:
  ```sql
...
- `N_M12` 出账借据编号与借据正常本金余额  pred_rows=100 gold_rows=100  err=result_mismatch
  ...
  ```
- `N_M13` 计划应还利息对比明细实还利息  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_ev_repay_detail.`int` AS `应还利息`, dwd_ev_repay_detail.compound_paid AS `实还利息` FROM dwd_ev_repay_detail LIMIT 100
  ```
  gold:
  ```sql
  SELECT DWD_EV_REPAY_PLAN.rpy_int AS `rpy_int`, DWD_EV_REPAY_DETAIL.`int` AS `int`, DWD_EV_REPAY_PLAN.term_no AS `term_no` FROM DWD_EV_REPAY_PLAN INNER JOIN dwd_ar_loan_info ON dwd_ar_loan_info.loan_no = dwd_ev_repay_plan.loan_no INNER JOIN dwd_ev_repay_detail ON dwd_ar_loan_info.loan_no = dwd_ev_repay_detail.loan_no LIMIT 100
  ```
- `N_M16` 户籍省份与名下借据逾期本金  pred_rows=100 gold_rows=100  err=result_mismatch
  pred:
  ```sql
  SELECT dwd_ip_indv_cust_info.hhdist AS `户籍省份`, dwd_ar_loan_info.ovd_bal AS `逾期本金` FROM dwd_ip_indv_cust_info INNER JOIN dwd_ar_loan_info ON dwd_ar_loan_info.cust_id = dwd_ip_indv_cust_info.cust_id LIMIT 100
  ```
  gold:
  ```sql
  SELECT DWD_IP_INDV_CUST_INFO.hhdist AS `hhdist`, DWD_IP_INDV_CUST_INFO.cust_id AS `cust_id`, DWD_AR_LOAN_INFO.ovd_bal AS `ovd_bal` FROM DWD_IP_INDV_CUST_INFO INNER JOIN DWD_AR_LOAN_INFO ON DWD_AR_LOAN_INFO.cust_id = DWD_IP_INDV_CUST_INFO.cust_id LIMIT 100
  ```
